    ...
    def check_command_completeness(cls, text, intent):
        ...
        if intent == 'UPDATE_MARKS' or intent == 'ENTER_MARKS':
            ...
            if not has_roll and not has_name:
                result['is_complete'] = False
                result['missing'].append('roll number or student name')

            if not result['is_complete']:
                result['suggestion'] = f"Missing: {', '.join(result['missing'])}. Example: 'Collect 5000 from Ishan Rao cash' or 'Collect 5000 from roll 12 class 6A cash'"
...
class EntityExtractor:
    ...
    def _extract_marks_entities(cls, text, context=None):
        """
        Extract entities for marks entry. Supports multi-student commands.

        Expected format examples:
        - "Enter marks for roll number 22, class 9B. Maths 85, Hindi 78, English 92"
        - "Update marks for student 1 maths 90 hindi 80"
        - "Update marks for student 2 maths 100 hindi 90 and also update marks for student 3 maths 100 hindi 90"
        """
        if context is None:
            context = {}
        ...
        # Check for multi-student command by counting student/roll references
        roll_matches = list(re.finditer(
            r'(?:student|roll|rule)[\.\s]+(?:number|no\.?|num|#)?[\.\s]*(\d+)',
            text, re.IGNORECASE
        ))

        if len(roll_matches) > 1:
            # Multi-student command — split and extract each
            logger.info(f"Multi-student command detected: {len(roll_matches)} students")
            return cls._extract_multi_student_marks(text, roll_matches, context)
    ...
    @classmethod
    def _extract_multi_student_marks(cls, text, roll_matches, context):
        """
        Extract entities for multi-student marks commands.
        Splits the text by student references and extracts marks for each.
        """
        students = []
        for i, match in enumerate(roll_matches):
            roll_number = int(match.group(1))
            # Get the text segment for this student (from this match to the next)
            start = match.start()
            end = roll_matches[i + 1].start() if i + 1 < len(roll_matches) else len(text)
        ...
        # Build entities with multi-student structure
        entities = {
            'multi_student': True,
            'students': students,
        }

        # Extract class/section from full text or context
        class_patterns = [r'class\s+(\d+)', r'grade\s+(\d+)']
        for pattern in class_patterns:
            match = re.search(pattern, text, re.IGNORECASE)
            if match:
                entities['class'] = int(match.group(1))
        ...
        section_patterns = [r'class\s+\d+\s*([A-Za-z])', r'section\s+([A-Za-z])']
        for pattern in section_patterns:
            match = re.search(pattern, text, re.IGNORECASE)
            if match:
                entities['section'] = match.group(1).upper()
        ...
        logger.info(f"Extracted multi-student marks entities: {entities}")
        return entities

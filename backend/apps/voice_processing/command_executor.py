...
class CommandExecutor:
    ...
    def prepare_confirmation(cls, intent, entities, user):
        ...
        if intent == 'BATCH_UPDATE_QUESTION_MARKS':
            ...
            if entities.get('multi_student'):
                return cls._prepare_multi_student_marks_confirmation(entities, user)
    ...
    def execute(cls, intent, entities, confirmation_data, user):
        ...
        if intent == 'BATCH_UPDATE_QUESTION_MARKS':
            ...
            if confirmation_data.get('multi_student'):
                return cls._execute_multi_student_marks(entities, confirmation_data, user)
    ...
    @classmethod
    def _prepare_multi_student_marks_confirmation(cls, entities, user):
        """
        Prepare confirmation for multi-student marks update.
        Processes each student and returns a combined confirmation.
        """
        ...
        students_data = []
        for student_entry in entities['students']:
            single_entities = {
                'roll_number': student_entry['roll_number'],
                'marks': student_entry['marks'],
                'class': entities['class'],
                'section': entities['section'],
            }
            try:
                confirmation = cls._prepare_marks_confirmation(single_entities, user)
                students_data.append(confirmation)
            except ValueError as e:
                logger.warning(f"Skipping student roll {student_entry['roll_number']}: {e}")
                continue

        if not students_data:
            raise ValueError("No valid students found in the command")
    ...
    @classmethod
    @transaction.atomic
    def _execute_multi_student_marks(cls, entities, confirmation_data, user):
        """
        Execute marks entry/update for multiple students.
        """
        results = []
        for student_data in confirmation_data['students_data']:
            result = cls._execute_marks_entry(entities, student_data, user)
        ...
        return {
            ...
            'message': f"Marks updated for {len(results)} students successfully",
        }

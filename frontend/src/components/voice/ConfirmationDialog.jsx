...
const ConfirmationDialog = () => {
  ...
  useEffect(() => {
    ...
    recognition.onresult = (event) => {
      ...
      // Check all results including interim for faster response
      let finalTranscript = ''
      ...
      console.log('[VoiceConfirm] Heard:', transcript, finalTranscript ? '(final)' : '(interim)')
      ...
      if (transcript.includes('confirm') || transcript.includes('yes') ||
          ...
          transcript.includes('proceed') || transcript.includes('do it') ||
          transcript.includes('go ahead') || transcript.includes('submit') ||
          transcript.includes('approve') || transcript.includes('sure') ||
          transcript.includes('haan') || transcript.includes('han') ||
          transcript.includes('theek hai') || transcript.includes('thik hai') ||
          transcript.includes('kar do') || transcript.includes('karo')) {
        actionTaken = true
      ...
      // Check for cancel commands
               ...
               transcript.includes('nahi') || transcript.includes('nah') ||
               transcript.includes('ruko') || transcript.includes('mat karo') ||
               transcript.includes('band karo') || transcript.includes('close')) {
        actionTaken = true
    ...
    recognition.onend = () => {
      // Auto-restart unless we intentionally stopped or took action
      if (!stopped && !actionTaken) {
        setIsListeningForConfirm(false)
        restartRecognition()
      }
    }
  ...
  const handleConfirm = async () => {
    try {
      ...
      if (intent === 'UPDATE_MARKS' || intent === 'ENTER_MARKS') {
        ...
        // Build list of student updates — handles both single and multi-student
        const studentUpdates = activeData.multi_student && activeData.students_data
          ? activeData.students_data
          : [activeData]
        ...
        studentUpdates.forEach(({ student, marks_table }) => {
          if (!student || !marks_table) {
            console.log('ERROR: Missing student or marks_table')
            return
          }
